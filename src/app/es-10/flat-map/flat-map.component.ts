...
export class FlatMapComponent implements OnInit {
  ...
  example2() {
    ...
    const number = [2, 3, 4];
    const mapOnly = number.map(num => [num, num ** 2, num ** 3]);
    console.log('mapOnly', mapOnly); // [[2,4,8], [3,9,18], [4,16,64]]

    const flatMapOnly = number.flatMap(num => [num, num ** 2, num ** 3]);
    console.log('flatMapOnly', flatMapOnly); // [2,4,8, 3,9,18, 4,16,64]
  }
